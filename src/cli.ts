...
const createSynthesisYargs = (name: string, description: string) => yargs => {
  return yargs
    .positional(name, {
      description,
      type: 'string'
...
async function main() {
  require(`yargs`)
    ...
    .command(
      ...
      createSynthesisYargs('inputText', 'The plain text to synthesize'),
      opts =>
        lib.synthesizeText(opts.inputText, opts.outputFile, {
    ...
    .command(
      ...
      createSynthesisYargs('ssmlText', 'The SSML text to synthesize'),
      opts =>
        lib.synthesizeSsml(opts.ssmlText, opts.outputFile, {
          name: opts.voiceProfile
        })
    )
    .command(
      ...
      createSynthesisYargs(
        'filePath',
        'The path to a plain text file to synthesize'
      ),
      opts =>
        lib.synthesizeTextFile(opts.filePath, opts.outputFile, {
          name: opts.voiceProfile
        })
    )
    .command(
      ...
      createSynthesisYargs(
        'filePath',
        'The path to a SSML text file to synthesize'
      ),
      opts =>
        lib.synthesizeSsmlFile(opts.filePath, opts.outputFile, {
          name: opts.voiceProfile
        })
    )
    .command(
      `list-voices`,
      'Prints all available voice profiles',
      {
        wavenet: {
          alias: 'w',
